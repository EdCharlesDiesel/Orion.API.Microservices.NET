...
namespace Orion.API.Person.Controllers
{
    ...
    [ApiController]
    public class AddressTypesController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.AddressTypes.GetAllAsync();
            return Ok(versions);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.AddressType version)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.AddressType version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.AddressTypes.GetByIdAsync(id);
                if (existing == null)
                {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.AddressTypes.Delete(existing);
                await unitOfWork.CompleteAsync();
