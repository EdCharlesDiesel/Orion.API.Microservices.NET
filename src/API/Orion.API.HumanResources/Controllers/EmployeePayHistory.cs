...
namespace Orion.API.HumanResources.Controllers
{
    ...
    [ApiController]
    public class EmployeePayHistoryController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.EmployeePayHistories.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.EmployeePayHistories.GetByIdAsync(id);
        ...
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.EmployeePayHistory version)
        {
            ...
            try
            {
                var existing = await unitOfWork.EmployeePayHistories.GetByIdAsync(id);
                if (existing == null)
                {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.EmployeePayHistories.Delete(existing);
                await unitOfWork.CompleteAsync();
